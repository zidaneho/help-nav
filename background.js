...
chrome.storage.sync.get(['claudeApiKey', 'fishAudioApiKey', 'fishAudioReferenceId'], (data) => {
  ...
  console.log('Fish Audio:', fishAudioApiKey ? 'Configured' : 'Not configured');
});
...
Analyze if this page is relevant to the user's goal. What action should be taken? Respond with JSON only.`;
  ...
  try {
    ...
    } else if (actionData.action === 'click' || actionData.action === 'highlight') {
...
async function handleCommand(text) {
  ...
  if (!claudeApiKey) {
    ...
    sendToActiveTab({ 
      type: 'SPEAK_ERROR', 
      message: 'Please configure your Claude API key in settings to use intelligent commands.' 
    });
    return;
  }
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
    ...
    if (tab.url.startsWith('chrome://') || tab.url.startsWith('chrome-extension://') || tab.url.startsWith('edge://')) {
      ...
      sendToActiveTab({ 
        type: 'SPEAK_ERROR', 
        message: 'Spotlight cannot work on Chrome special pages. Please navigate to a regular website.' 
      });
      return;
    }
    ...
    try {
      ...
      if (!response) {
        console.error('Could not get DOM snapshot - content script may not be loaded');
        sendToActiveTab({ 
          type: 'SPEAK_ERROR', 
          message: 'Please refresh the page and try again. Spotlight needs to load on this page first.' 
        });
        return;
      }
      ...
      const action = await queryClaudeForAction(text, response.domSnapshot, response.url);
      ...
      sendToActiveTab({ 
        type: 'SPEAK_ERROR', 
        message: 'Sorry, I encountered an error processing your command. Check the console for details.' 
      });
    }
  });
}
    ...
    if (tabs.length) {
      chrome.tabs.sendMessage(tabs[0].id, message, (response) => {
        ...
        if (chrome.runtime.lastError) {
          ...
          console.log('This usually means the content script is not loaded on this page.');
        }
      });
    }
  });
...
chrome.runtime.onMessage.addListener((msg, sender) => {
  ...
  if (msg.type === 'RELOAD_FISH_AUDIO_CONFIG' || msg.type === 'RELOAD_CONFIG') {
    ...
    chrome.storage.sync.get(['claudeApiKey', 'fishAudioApiKey', 'fishAudioReferenceId'], (data) => {
      ...
      fishAudioApiKey = data.fishAudioApiKey || null;
      fishAudioReferenceId = data.fishAudioReferenceId || null;
      console.log('Config reloaded - Claude:', claudeApiKey ? 'Yes' : 'No', 'Fish Audio:', fishAudioApiKey ? 'Yes' : 'No');
    });
    ...
    (async () => {
      try {
        ...
        chrome.tabs.sendMessage(sender.tab.id, {
          ...
          if (chrome.runtime.lastError) {
            console.log('Could not send TTS audio to tab:', chrome.runtime.lastError.message);
          }
        });
      } catch (error) {
        ...
        chrome.tabs.sendMessage(sender.tab.id, {
          ...
          if (chrome.runtime.lastError) {
            console.log('Could not send TTS error to tab:', chrome.runtime.lastError.message);
          }
        });
      }
    })();
