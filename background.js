...
chrome.storage.sync.get(['fishAudioApiKey', 'fishAudioReferenceId'], (data) => {
  ...
  console.log('Fish Audio config loaded:', fishAudioApiKey ? 'API key present' : 'No API key');
});
...
function handleCommand(text) {
  ...
  if (text.includes('scroll down')) {
    ...
    console.log('handleCommand: Extracted target from "click" command:', target);
    payload = { action: 'click', selector: target, speak: `Here's the ${target} button. Click it to proceed.` };
  } else if (text.includes('highlight')) {
    const target = text.split('highlight')[1]?.trim();
    console.log('handleCommand: Extracted target from "highlight" command:', target);
    payload = { action: 'highlight', selector: target, speak: `Highlighting ${target}.` };
